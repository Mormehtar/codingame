...
class Field:
    ...
    def update(self, position, field_type, obj=None):
        ...
        if field_type == CRATE or field_type == "1" or field_type == "2":
            field_type = CRATE
            if obj is None:
            ...
            for direction in DIRECTIONS:
                ...
                for i in range(bomb.bomb_range - 1):
                    ...
                    if local_field.type == BOMB or local_field.type == WALL:
                        break
                    if bomb.timer <= BOMB_TRIGGER:
                        local_field.potential -= (BOMB_TRIGGER - bomb.timer) * BOMB_DANGER
                    else:
            ...
            for direction in DIRECTIONS:
                ...
                for i in range(me.bomb_range - 1):
                    ...
                    if position is None:
                        break
                    local_field = self.field[position]
                    if local_field.type == CRATE or local_field.type == WALL:
                        break
